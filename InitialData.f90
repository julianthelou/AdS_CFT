...
RECURSIVE SUBROUTINE InitParameters(STRLEN,PARSETUP) 
    USE ISO_C_BINDING
    USE MainVariables, ONLY: nVar , nDim, EQN, ICType, NSTOV_rho_atmo, NSTOV_kappa,NSTOV_p_atmo, istwopunct, isAdS3
    USE NSTOV_mod
    IMPLICIT NONE  
    integer          :: STRLEN
    ...
    case('CCZ4TwoPunctures')
        ...
        EQN%CCZ4GLMc = 0.8
        EQN%CCZ4GLMd = 0.02
        istwopunct = LOGICAL( .TRUE., KIND=C_BOOL )
    ...
    case('AdS3')
        ! We will set the parameters for the moment as for the Gauge wave 
        EQN%CCZ4k1  = 0.0
        EQN%CCZ4k2  = 0.0 
        EQN%CCZ4k3  = 0.0 
        EQN%CCZ4eta = 0.0 
        ...
        EQN%CCZ4bs  = 0.0
        EQN%CCZ4LapseType   = 0 ! harmonic lapse
        isAdS3 = LOGICAL( .TRUE., KIND=C_BOOL )
...
RECURSIVE SUBROUTINE InitialData(xGP, tGp, Q)
    ...
    ! The compactified radial coordiante rho = r/(1+r)
     AdSrho = xGP_sph(1)/(1+xGP_sph(1))
        
    ! The scalar field q = 1-rho
    AdSq = 1-AdSrho
    
    ! The auxiliary variable a = q^2 + rho^2
    AdSa = AdSq**2 + AdSrho**2
    ...
    DAdSa = -2 + 4*AdSrho
        ...
        ! The conformally decomposed spatial metric 
        V0(1)  = 1
        ! V0(1)  = 1/(SQRT(AdSa)*(AdSrho))
        !V0(4)  = SQRT(AdSa)*AdSrho
        V0(4)  = 1
        
        ! The conformally decomposed traceless-part of the extrinsic curvature
        
        ! Theta
        
        ! Gamma-hat
        V0(14) = 1
        !V0(14) = -0.5*AdSrho/SQRT(AdSa)*DAdSa - SQRT(AdSa)
        
        ! Logarithm of the lapse function
        ! V0(17) = LOG(1-AdSq) + 0.5*LOG(1+AdSq**2/AdSrho**2) - LOG(AdSq) 
        V0(17) = 1
        ...
        ! D_ijk
        V0(36) = 1
        ! V0(36) = -1/(2*(AdSa)**(1.5)) + AdSq/(2*(AdSrho)*(AdSa)**(1.5)) - 1/(2*SQRT(AdSa)*(AdSrho)**2)
         
        ! V0(39) = AdSrho**2/(2*SQRT(AdSa)) - AdSrho*AdSq / (2*SQRT(AdSa)) + 0.5*SQRT(AdSa) 
        V0(39) = 1
                
        ! The trace K of the extrinsic curvature K_ij
        
        ! The logarithm of the conformal factor phi
        V0(55) = 1
        !V0(55) = LOG(AdSq) + 0.25*LOG(AdSa) - 0.5*LOG(AdSrho)
        
        ! P_i
        ! V0(56) = -1/(AdSq) + AdSrho/(2*(AdSa)) - AdSq/(2*(AdSa)) - 1/(2*(AdSrho))
        V0(56) = 1
        
        ! Trace K of the extrinsic curvature K_ij at time t=0
